...
## ROAS Event Tracking (BẮT BUỘC)

> **⚠️ MANDATORY**: Mỗi khi tích hợp bất kỳ loại quảng cáo AdMob nào (Banner, Interstitial, Rewarded, AppOpen, Native), bạn **PHẢI** đồng thời implement ROAS event tracking.

*   **ALWAYS** read and follow the `admob-roas` skill (`~/.gemini/antigravity/skills/admob-roas/SKILL.md`) khi implement quảng cáo.
*   **ALWAYS** attach `OnPaidEventListener` (Android) hoặc `paidEventHandler` (iOS) ngay sau khi ad load thành công.
*   **ALWAYS** log event `ad_impression` về Firebase Analytics với đầy đủ parameters: `ad_platform`, `ad_source`, `ad_format`, `value`, `currency`.
*   **NEVER** implement quảng cáo mà không có ROAS tracking — sản phẩm sẽ không thể chạy tROAS campaign trên Google Ads.

### Related Skills
- `admob-roas` — Chi tiết implement ROAS tracking cho Android (Kotlin) và iOS (Swift)